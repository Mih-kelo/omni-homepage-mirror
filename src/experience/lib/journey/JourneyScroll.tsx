...
export function JourneyScroll({
  ...
  useLayoutEffect(() => {
    ...
    // The conductor lives on refs owned by an ancestor (.lumen) and a later
    // sibling (.lumen-track). On a normal hydration ClientOnly mounts us in a
    // SECOND commit, so those refs are already attached when this runs. But
    // when SSR bails inside a Suspense boundary (React #419, seen only in the
    // production build) the subtree is client-rendered in ONE commit and refs
    // attach as that commit unwinds — so on the first run they can still be
    // null. Retry next frame instead of permanently giving up, or the entire
    // scroll pipeline (grade/spotlight, the drawn thread, the thread-written
    // headlines — everything downstream of journey progress) stays dead for
    // the whole session.
    const start = () => {
      const root = rootRef.current;
      const track = trackRef.current;
      if (!root || !track) {
        if (!disposed) startRaf = requestAnimationFrame(start);
        return;
      }
      ...
      t2 = window.setTimeout(refresh, 1600);
